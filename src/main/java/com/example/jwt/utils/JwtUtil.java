...
public class JwtUtil {
	...
	public  String generateToken(String userId,String username,String firstName,String lastName,boolean isAdmin){
		Date now = new Date();
	...
	public String getUserIdFromRequest(HttpServletRequest request){
		if(request != null){
			...
			if (StringUtils.hasText(bearerToken)) {
				...
				String result = getClaimsFromJWT(bearerToken).get("userId").toString();
				//获取结束时间
	...
	public String getUserIdFromRequest(String token){
		if (StringUtils.hasText(token)) {
			long startTime=System.currentTimeMillis();
			String result = getClaimsFromJWT(token).get("userId").toString();
			//获取结束时间
			long endTime=System.currentTimeMillis();
			logger.info("Token解析时间 :{} ms",(endTime-startTime));
			return result;
	...
	public static void main(String[] args) {
		...
		System.out.println(jwt.generateToken("111", "hello", "test", "test", true));
	}
}
